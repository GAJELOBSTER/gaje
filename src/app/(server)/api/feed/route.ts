 ...
 *              properties:
 *                feed:
 *                  $ref: '#/definitions/schema/FeedWithFeedItemWithAction'
 *      '400':
 ...
 *                      <font color="#ff4f4f">RSS_URL_ALREADY_REGISTERED</font> - 이미 등록된 Feed URL
 *         content:
 *           application/json:
 *             schema:
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (checkFeed) return handleError("RSS_URL_ALREADY_REGISTERED");
    ...
    const convertedFeedItem = createdFeed.feedItem.map((item: FeedItem) => {
      return { ...item, isBookmark: false, isHidden: false };
    });

    return NextResponse.json({ ...createdFeed, feedItem: convertedFeedItem }, { status: 201 });
  } catch (error) {
