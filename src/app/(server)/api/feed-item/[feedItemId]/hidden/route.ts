...
/**
 * @swagger
 * /api/feed-item/{feedItemId}/hidden:
 *  post:
 *    tags: [Feed Item]
 *    summary: 피드 아이템 숨김 토글
 *    description: 숨김 추가 - 201 반환 </br>
 *                 숨김 제거 - 204 반환
 *    parameters:
 *      - in: path
 *        name: feedItemId
...
export async function POST(req: NextRequest, { params }: { params: Params }) {
  try {
    ...
    const checkFeedAction = await prisma.feedAction.findUnique({
      where: {
        userId_feedItemId_type: { userId: authResult.user.id, feedItemId: paramsData.feedItemId, type: "HIDDEN" },
      },
    });
    ...
    if (checkFeedAction) {
      ...
      const createdFeedAction = await prisma.feedAction.create({
        data: { userId: authResult.user.id, feedItemId: paramsData.feedItemId, type: "HIDDEN" },
      });
